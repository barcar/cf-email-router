...
3. Set the **`EMAIL_ROUTES`** secret to a JSON array (dashboard: Worker → Settings → Variables and secrets, or `npx wrangler secret put EMAIL_ROUTES`). Put a catch-all route last using `"to": "*"`.
   ...
   **Plus addressing (`local+tag@domain`).** Cloudflare calls this **subaddressing** ([RFC 5233](https://www.rfc-editor.org/rfc/rfc5233)). Enable it in the dashboard under **Email → Email Routing → Settings** (not only under Routes). Once it is on, mail to `user+tag@yourdomain` is handled by the **same Email Routing rule** as `user@yourdomain`—you do not add a separate custom address for each tag—while the `+tag` detail can still appear in the envelope for this worker. See Cloudflare’s [Subaddressing](https://developers.cloudflare.com/email-routing/setup/email-routing-addresses/#subaddressing) section for behavior and edge cases (for example, an explicit `user+tag@…` rule overrides the base `user@…` rule). This worker still matches **`to` exactly** in **`EMAIL_ROUTES`**: `apollo+kuma@yourdomain` does **not** match a route for `apollo@yourdomain`; add a separate route for each tagged address you want to treat differently. To mirror another mailbox’s Discord notifications, duplicate that route’s **`discordWebhookUrl`** and set **`forwardTo`** to the inbox that should receive the tagged mail. A domain **catch-all** rule is separate: use it when you want *any* local part delivered to the worker without listing each base address in Email Routing.
